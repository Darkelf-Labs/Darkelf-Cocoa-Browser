# Darkelf Cocoa Browser v4.4.3 — Ephemeral, Privacy-Focused Web Browser (macOS / Cocoa Build)
...
"""
    ...
    def _layout_toolbar(self):
        ...
        right_gap = 14.0          # spacing between right icons
        urlbar_gap = 18.0         # breathing room between URL bar and keyboard icon

        right_cluster_width = (
            len(right_buttons) * btn +
            (len(right_buttons) - 2) * right_gap
        )
        ...
        for b in right_buttons:
            b.setFrame_(NSMakeRect(x_cursor, 10, btn, btn))
            x_cursor += btn + right_gap

        # URL bar
        url_x = left_end
        url_w = (right_x - urlbar_gap) - url_x
